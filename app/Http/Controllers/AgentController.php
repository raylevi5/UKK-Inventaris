...
class AgentController extends Controller
{
    ...
    {
        // Eager load relationships and sort by date
        $hitungDepresiasis = HitungDepresiasi::with(['pengadaan.masterBarang'])
            ->orderBy('tgl_hitung_depresiasi', 'desc')
            ->get();
        
        // Calculate depreciation for each item
        $hitungDepresiasis->each(function($depresiasi) {
            if ($depresiasi->pengadaan && $depresiasi->pengadaan->masterBarang) {
                $depresiasi->perhitungan = $depresiasi->hitungSisaDepresiasi();
            }
        });

        return view('agent.hitung_depresiasi', compact('hitungDepresiasis'));
    }
    ...
    public function detailPengadaan($id)
{
    ...
    return view('agent.detail_pengadaan', compact('pengadaan'));
}

public function detailHitungDepresiasi($id)
{
    $hitungDepresiasi = HitungDepresiasi::with(['pengadaan.masterBarang'])->findOrFail($id);
    $hitungDepresiasi->perhitungan = $hitungDepresiasi->hitungSisaDepresiasi();
    
    return view('agent.detail_hitung_depresiasi', compact('hitungDepresiasi'));
}
}