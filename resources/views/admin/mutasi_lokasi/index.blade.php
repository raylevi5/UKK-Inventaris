...
<head>
    ...
    <title>Data Mutasi Lokasi</title>
...
<body>
    ...
    <div class="main-content">
        <h1>Data Mutasi Lokasi</h1>
        <a href="{{ route('admin.mutasi_lokasi.create') }}" class="add-item-link">Tambah Mutasi Lokasi</a>
            ...
            <tbody>
                @foreach ($mutasiLokasis as $mutasi)
                    <tr>
                        ...
                        <td>{{ $mutasi->pengadaan->masterBarang->nama_barang }}</td>
                        <td>{{ $mutasi->flag_lokasi }}</td>
                        <td>{{ $mutasi->flag_pindah }}</td>
                        <td>
                            <a href="{{ route('admin.mutasi_lokasi.edit', $mutasi->id_mutasi_lokasi) }}">Edit</a>
                            <form action="{{ route('admin.mutasi_lokasi.destroy', $mutasi->id_mutasi_lokasi) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
